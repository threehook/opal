...
def _discover_store_id(retries: int = 20, delay_seconds: float = 1.0) -> str:
    ...
    start-openfga.sh creates a fresh OpenFGA store per container start
    (see docker-compose-app-tests-openfga.yml), so the id isn't known up
    front - it has to be discovered from the running server instead of
    hardcoded.
    """
