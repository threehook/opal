...
def init_data_updates_router(
    ...
    async def default_all_data():
        ...
        try:
            ...
            # The policy repo clone happens in the background, so the file may not exist yet on the first request after
            # server startup. Retry for a few seconds, staying under the client's default 5s fetch timeout - the store write
            # is retried on failure, but always with this same fetched value, so it has to be correct on this request.
            data_file = None
            attempts, delay_seconds = 8, 0.5
            for attempt in range(attempts):
                data_file = find_data_file(clone_path, data_filename)
                if data_file or attempt == attempts - 1:
                    break
